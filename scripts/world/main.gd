...
## in, then drops the starting colonists on it.
...
var _colonists_spawned: bool = false


func _process(_delta: float) -> void:
	if _colonists_spawned:
		return
	...
	if not world.is_area_meshed(AABB(Vector3(site) - Vector3.ONE * 16.0, Vector3.ONE * 32.0)):
		return
	colony.spawn_initial_colonists(site)
	_colonists_spawned = true
